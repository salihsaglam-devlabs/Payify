...
public class TransactionsController : ApiControllerBase
{
    ...
    /// <summary>
    /// Returns all transactions list of the Custody Account's Wallet
    /// </summary>
    /// <param name="request"></param>
    [Authorize(Policy = "Transaction:ReadAll")]
    [HttpGet("getCustodyWalletTransactions")]
    public async Task<PaginatedList<TransactionDto>> GetCustodyWalletTransactionsAsync(
        [FromQuery] GetCustodyWalletTransactionsRequest request)
    {
        return await _transactionHttpClient.GetCustodyWalletTransactionsAsync(request);
    }
    ...
    {
        return await _transactionHttpClient.GetUserFastTransactionAmountsAsync(id);
    }
}