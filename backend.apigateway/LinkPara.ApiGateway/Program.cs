...
using LinkPara.ApiGateway.Authorization;
using LinkPara.ApiGateway.Authorization.CustomAuthorizations;
using LinkPara.ApiGateway.Authorization.CustomPolicies.OtpPolicy;
using LinkPara.ApiGateway.Authorization.Scopes;
using LinkPara.ApiGateway.Commons;
using LinkPara.ApiGateway.Commons.EventBusConfiguration;
using LinkPara.ApiGateway.Commons.Helpers;
using LinkPara.ApiGateway.Commons.Models;
using LinkPara.ApiGateway.Filters.RequestResponseLogging;
...
builder.Services
    .AddControllers(options =>
    {
        ...
        options.Filters.Add<LinkPara.ApiGateway.Authorization.CustomAuthorizations.AuthorizeAttribute>();
    })
...
{
    option.SwaggerDoc("v1", new OpenApiInfo { Title = "Gateway API", Version = "v1" });
...
builder.Host.UseSerilog((ctx, lc) =>
{
    lc.WriteTo.Logger(new LoggerConfiguration()
        ...
        .Enrich.WithThreadId()
        .Enrich.WithProperty("ApplicationName", "LinkPara.ApiGateway")
        .WriteTo.Debug()
        .WriteTo.RabbitMQ(
...
builder.Services.AddScoped<IPaymentOtpRequirementService, PaymentOtpRequirementService>();
...
configuration.AddInMemoryCollection(
    new Dictionary<string, string>
    {
        ...
        { "ElasticApm:ServiceName", "ApiGateway" }
    });
