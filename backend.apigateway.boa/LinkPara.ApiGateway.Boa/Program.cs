...
using LinkPara.ApiGateway.Boa.Authorization.BasicAuthorizations;
using LinkPara.ApiGateway.Boa.Commons;
using LinkPara.ApiGateway.Boa.Commons.EventBusConfiguration;
using LinkPara.ApiGateway.Boa.Commons.Helpers;
using LinkPara.ApiGateway.Boa.Commons.Models;
using LinkPara.ApiGateway.Boa.Filters.CustomerContext;
using LinkPara.ApiGateway.Boa.Filters.RequestResponseLogging;
...
{
    c.SwaggerDoc("v1", new OpenApiInfo { Title = "BOA Gateway API", Version = "v1" });
...
builder.Host.UseSerilog((ctx, lc) =>
{
    lc.WriteTo.Logger(new LoggerConfiguration()
        ...
        .Enrich.WithThreadId()
        .Enrich.WithProperty("ApplicationName", "LinkPara.ApiGateway.Boa")
        .WriteTo.Debug()
        .WriteTo.RabbitMQ(
...
configuration.AddInMemoryCollection(
    new Dictionary<string, string>
    {
        ...
        { "ElasticApm:ServiceName", "ApiGateway.Boa" }
    });
...
app.UseGatewayRateLimiter(vaultClient.GetSecretValue<RateLimiterSettings>("SharedSecrets", "RateLimitingSettings"),
                          Gateway.BoaApiGateway.ToString(),
