...
public class IngestionClearingBkmDetailConfiguration : IEntityTypeConfiguration<IngestionClearingBkmDetail>
{
    public void Configure(EntityTypeBuilder<IngestionClearingBkmDetail> builder)
    {
        ...
        builder.HasOne(x => x.IngestionFileLine)
            .WithOne(x => x.ClearingBkmDetail)
            .HasForeignKey<IngestionClearingBkmDetail>(x => x.IngestionFileLineId);
    }

    internal static void ConfigureColumns<T>(EntityTypeBuilder<T> builder) where T : IngestionClearingBkmDetail
    {
