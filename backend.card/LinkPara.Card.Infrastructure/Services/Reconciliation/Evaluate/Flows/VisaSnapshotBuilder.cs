...
internal static class VisaSnapshotBuilder
{
        ...
        params (string Key, object? Value)[] extra)
    {
        ...
        AddGroup(payload, operationCode,
            ...
            ("isSettlementReceived", detail.IsTxnSettle == CardVisaIsTxnSettle.Settled),
    ...
    private static CardVisaDetail? DeserializeRootCardDetail(IngestionFileLine row)
    {
        ...
        try
        {
            return JsonSerializer.Deserialize<CardVisaDetail>(row.ParsedContent, JsonOptions);
        }
        catch
        {
            return null;
