...
using LinkPara.Emoney.Application;
using LinkPara.Emoney.Infrastructure;
...
using LinkPara.Emoney.Application.Commons.Models.ElasticApmConfiguration;
...
{
    options.SwaggerDoc("v1", new OpenApiInfo { Title = "Emoney API", Version = "v1" });
...
builder.Host.UseSerilog((ctx, lc) =>
{
    lc.WriteTo.Logger(new LoggerConfiguration()
        ...
        .Enrich.WithThreadId()
        .Enrich.WithProperty("ApplicationName", "Emoney.API")
        .WriteTo.Debug()
        .WriteTo.RabbitMQ(
...
configuration.AddInMemoryCollection(
    new Dictionary<string, string>
    {
        ...
        { "ElasticApm:ServiceName", "Emoney.API" }
    });
