...
using LinkPara.CampaignManagement.Application;
using LinkPara.CampaignManagement.Infrastructure;
...
using LinkPara.CampaignManagement.Application.Commons.Models.ElasticApmConfiguration;
...
{
    options.SwaggerDoc("v1", new OpenApiInfo { Title = "CampaignsManagement API", Version = "v1" });
...
builder.Services.AddJwtProviderAuthorization(vaultClient);
...
builder.Host.UseSerilog((ctx, lc) =>
{
    lc.WriteTo.Logger(new LoggerConfiguration()
        ...
        .Enrich.WithThreadId()
        .Enrich.WithProperty("ApplicationName", "CampaignManagement.API")
        .WriteTo.Debug()
...
configuration.AddInMemoryCollection(
    new Dictionary<string, string>
    {
        ...
        { "ElasticApm:ServiceName", "CampaignManagement.API" }
    });
