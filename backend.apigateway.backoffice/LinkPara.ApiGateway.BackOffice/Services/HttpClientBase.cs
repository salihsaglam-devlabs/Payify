...
namespace LinkPara.ApiGateway.BackOffice.Services;

public class HttpClientBase
{
    private readonly HttpClient _client;
    private bool _canSeeSensitiveData;
    protected HttpClientBase(HttpClient client, IHttpContextAccessor httpContextAccessor)
    {
        ...
        if (httpContext is not null)
        {
            ...
            var canSeeSensitiveData = httpContext.User?.FindAll("CanSeeSensitiveData");

            _canSeeSensitiveData = (bool)canSeeSensitiveData?.Any(claim => bool.TryParse(claim.Value, out var value) && value);

            _client.DefaultRequestHeaders.Add("CanSeeSensitiveData", _canSeeSensitiveData.ToString());      
        ...
        _client.DefaultRequestHeaders.Add("Gateway", Gateway.BackOffice.ToString());
        _client.DefaultRequestHeaders.Add("CorrelationId", Guid.NewGuid().ToString());

    }

    protected bool CanSeeSensitiveData()
    {
        return _canSeeSensitiveData;
    }
        ...
        Dictionary<string, string> headers)
    {
        ...
        var result = await PostAsync(requestUri, jsonValue);
    ...
    protected static async Task HandleExceptionAsync(HttpResponseMessage response)
    {
        ...
        await HandleStandardException(response);
    }
    ...
    private static async Task HandleStandardException(HttpResponseMessage response)
    {
        if (response.StatusCode == HttpStatusCode.Unauthorized)
        { 
            ...
            try
            {
                ...
                Console.WriteLine($"HandleStandardException message {message}");

            }
            catch(Exception e)
            {
                Console.WriteLine($"HandleStandardException ex message {e}");
                // will be removed
            }
