...
public class MerchantUserHttpClient : HttpClientBase, IMerchantUserHttpClient
{
    public MerchantUserHttpClient(HttpClient client, IHttpContextAccessor httpContextAccessor) 
    ...
    public async Task<MerchantUserNameModel> GetUserNameAsync(string identifier)
    {
        var response = await GetAsync($"v1/MerchantUsers/username/{identifier}");
        var merchantUserName = await response.Content.ReadFromJsonAsync<MerchantUserNameModel>();
        return merchantUserName ?? throw new InvalidOperationException();
    }
}
