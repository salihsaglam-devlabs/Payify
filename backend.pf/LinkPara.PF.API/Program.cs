...
using LinkPara.PF.Application.Commons.Models.ElasticApmConfiguration;
...
{
    options.SwaggerDoc("v1", new OpenApiInfo { Title = "PF API", Version = "v1" });
...
builder.Services.AddPostingBatches();
builder.Services.AddJwtAuthorization(vaultClient);
builder.Services.ConfigureSeedData();

var eventBusSettings = await vaultClient.GetSecretValueAsync<EventBusSettings>("SharedSecrets", "EventBusSettings", null);
...
builder.Host.UseSerilog((ctx, lc) =>
{
    lc.WriteTo.Logger(new LoggerConfiguration()
        ...
        .Enrich.WithThreadId()
        .Enrich.WithProperty("ApplicationName","PF.API")
        .WriteTo.Debug()
        .WriteTo.Console()
        .WriteTo.RabbitMQ(
