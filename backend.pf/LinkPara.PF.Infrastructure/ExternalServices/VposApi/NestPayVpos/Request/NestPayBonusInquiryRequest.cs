...
public class NestPayBonusInquiryRequest : NestPayBaseRequest
{
    ...
    public string BuildRequest()
    {
        ...
        requestXml.AppendLine($"<Type>{OrderType.Trim()}</Type>");
        requestXml.AppendLine($"<Number>{Pan.Trim()}</Number>");
        requestXml.AppendLine($"<Expires>{Expiry}</Expires>");
        requestXml.AppendLine($"<Cvv2Val>{Cvv.Trim()}</Cvv2Val>");
        requestXml.AppendLine($"<Currency>{CurrencyCode}</Currency>");
        requestXml.AppendLine("<Extra>");
        requestXml.AppendLine($"<MAXIPUANSORGU>{QueryName.Trim()}</MAXIPUANSORGU>");
        requestXml.AppendLine("</Extra>");
        requestXml.AppendLine("</CC5Request>");
