...
public class DesignWithoutDue : IDocument
{
    public StatementDetails StatementModel { get; }

    public DesignWithoutDue(StatementDetails statementDetails)
    {
        ...
        container.Column(column =>
        {
            column.Item().Table(table =>
            {
                ...
                foreach (var item in StatementModel.Transactions)
                {
                    ...
                    table.Cell().Element(CellStyle).Padding(3).AlignMiddle().Text(item.CardNumber).FontSize(7);
                    ...
                    table.Cell().Element(CellStyle).Padding(3).AlignMiddle().Text((item.ChargebackAmount + item.DueAmount).ToString("F2")).FontSize(7);
