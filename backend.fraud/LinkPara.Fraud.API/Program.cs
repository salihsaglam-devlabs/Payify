...
using LinkPara.Fraud.Application;
using LinkPara.Fraud.Infrastructure;
...
using LinkPara.Fraud.Application.Commons.Models.ElasticApmConfiguration;
...
{
    options.SwaggerDoc("v1", new OpenApiInfo { Title = "Fraud API", Version = "v1" });
...
builder.Host.UseSerilog((ctx, lc) =>
{
    lc.WriteTo.Logger(new LoggerConfiguration()
        ...
        .Enrich.WithThreadId()
        .Enrich.WithProperty("ApplicationName","Fraud.API")
        .WriteTo.Debug()
        .WriteTo.RabbitMQ(
...
configuration.AddInMemoryCollection(
    new Dictionary<string, string>
    {
        ...
        { "ElasticApm:ServiceName", "Fraud.API" }
    });
