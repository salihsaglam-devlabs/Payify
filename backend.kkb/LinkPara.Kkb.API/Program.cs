...
using LinkPara.Kkb.Application.Commons.Models.ElasticApmConfiguration;
...
{
    options.SwaggerDoc("v1", new OpenApiInfo { Title = "KKB API", Version = "v1" });
...
builder.Host.UseSerilog((ctx, lc) =>
{
    lc.WriteTo.Logger(new LoggerConfiguration()
        ...
        .Enrich.WithThreadId()
        .Enrich.WithProperty("ApplicationName","Kkb.API")
        .WriteTo.Debug()
        .WriteTo.RabbitMQ(
...
configuration.AddInMemoryCollection(
    new Dictionary<string, string>
    {
        ...
        { "ElasticApm:ServiceName", "Kkb.API" }
    });
